...
function sumOfCodes(str) {
  ...
  const iter = (acc, arr) => {
    ...
    const newAcc = acc + arr.pop().charCodeAt();
    return iter(newAcc, arr);
  };
  return iter(0, str.split(''));
}
...
function isPalindrome(str) {
  const str1 = str
    ...
    .toLowerCase();
  return (
    str1.replace(/[^A-Z\d]/gi, '') ===
    str.replace(/[^A-Z\d]/gi, '').toLowerCase()
  );
}
...
function extractNameFromTemplate(value) {
  ...
  const lastName = arr[2].substring(0, arr[2].length - 1);
...
function isLetter(charcode) {
  ...
  const result = !!(
    (charcode >= maxUpperCase && charcode <= minUpperCase) ||
